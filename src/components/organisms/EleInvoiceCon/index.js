...
import { num_format,DX,cmp } from './../../util'
...
class EleInvoiceConMack extends React.Component{
  constructor(props){
    super(props);
    this.startState={
      line:[{
  ...
  handleSubmit = () => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if(!err){
        ...
        const param = {
          is_special: '0',
          issue_type: this.props.settingList[0].tax_control_type === 1?'1':'2',
        ...
        this.setState({previewMoal:<PreviewMoal rpaOpen={this.props.rpaOpen} 
                                                ele={true} 
        ...
        // this.props.saveEler(param,(res)=>{
        //   this.props.getDetail(res.data.outinvoice_id,(detail)=>{
        //     this.setState({showInvoice:false,loading:false},()=>{
  ...
  routerWillLeave= () => {
    ...
    if(state.bz !== startState.bz || state.skr !== startState.skr || state.fhr !== startState.fhr || state.tel !== startState.tel || 
      ...
      state.gfsbh !== startState.gfsbh || state.gfmc !== startState.gfmc || state.email !== startState.email){
    ...
    return(
      <div>
      {
        ...
        <InvoiceHeader>
          <HeaderLeft>
            ...
            <Select style={{width:150}} defaultValue='1'>
              <Option key='1'>增值税电子普通发票</Option>
            </Select>
          </HeaderLeft>
          {
            this.state.fjhList.length>0 &&
            <HeaderLeft>
              分机号：
                <Select style={{width:150}} onChange={this.fjhChange} defaultValue={`${this.state.fjhList[0]}`}>
                  {
                    this.state.fjhList.map((item)=>{
                      return <Option key={item}>{item}号机</Option>
                    })
                  }
                </Select>
            </HeaderLeft>
              ...
              <RightAspect>
                <div style={{overflow:'hidden'}}>
                  <InvoiceTitSpan><HaveToSpan> * </HaveToSpan>抬头类型：</InvoiceTitSpan>
                    <ConRadio defaultValue='company' value={this.state.headertype} onChange={(e)=>{
                      ...
                      <Radio value='person'>个人/事业单位</Radio>
                    </ConRadio>
                </div>
                <div style={{overflow:'hidden'}}>
                ...
                <div style={{overflow:'hidden'}}>
                  <InvoiceTitSpan>{this.state.headertype === 'company' && <HaveToSpan> * </HaveToSpan>}纳税人识别号：</InvoiceTitSpan>
                  <FormItem>
                    {getFieldDecorator('gfsbh',{
                      initialValue:this.state.gfsbh,
                      rules:[this.state.headertype === 'company'?{
                        required: true, message: '纳税人识别号不能为空',
                      }:'',{
                        pattern: /^[A-Z0-9]{15,20}$/, message: '纳税人识别号不合法'
                      }]
                    })(
                      <ConInput placeholder='请输入纳税人识别号' onChange={(e)=>this.setState({nsrsbh:e.target.value})}/>
                    )}
                  </FormItem>
                </div>
                <div style={{overflow:'hidden'}}>
                            ...
                            <EachCol style={{width:'25%',padding:5}}>
                              ...
                              {getFieldDecorator(`${item.id+'hwmc'}`,{
                                ...
                                <Search style={{width:'80%'}}
                                        ...
                                        onSearch={()=>{
                                          this.setState({codingModal:<CodingModal getCodingList={this.props.getCodingList}
                                                                                  cancel={()=>this.setState({codingModal:''})}
        ...
        <BottomCon>
          ...
          <BottomBtn type='primary' loading={this.state.loading} onClick={this.handleSubmit}>开票预览</BottomBtn>
        </BottomCon>
...
const EleInvoiceCon = Form.create()(EleInvoiceConMack);
export default EleInvoiceCon;